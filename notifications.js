...
// Node.js < 22 has no native WebSocket global, and @supabase/realtime-js throws
// synchronously inside createClient() (not just when a channel is opened) if it
// can't find one — matches server.js's own client, which already passes this.
// Confirmed by reproducing the actual Railway crash locally under real Node 20
// (this repo pins engines.node = "20.x"): omitting this option crashes the
// process at require-time, before it ever binds to a port.
...
// Hardcoded trusted constant, never derived from user/database input — safe to
// place directly in an href without escaping.
...
function rentDueReminderEmail({ tenantName, propertyName, amount, dueDate, label }) {
  ...
  const t = escapeHtml(tenantName), p = escapeHtml(propertyName);
  const l = escapeHtml(label), a = escapeHtml(amount), d = escapeHtml(dueDate);
  const html = wrap(`
    <p>Hi ${t},</p>
    <p><b>${l}</b> of <b>₹${a}</b> is due on <b>${d}</b> for ${p}.</p>
